...
class Settings(ttk.Frame):
    def __init__(self, parent, controller, show_timer):
        ...
        working_label = ttk.Label(
            settings_container,
            text="Working: ",
            style="LightText.TLabel"
        )
        working_label.grid(column=0, row=0, sticky="W")

        working_input = tk.Spinbox(
            settings_container,
            from_=0,
            to=120,
            increment=1,
            justify="center",
            textvariable=controller.timer,
            width=10,
        )
        working_input.grid(column=1, row=0, sticky="EW")
        working_input.focus()
