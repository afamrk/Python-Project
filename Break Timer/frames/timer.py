...
class Timer(ttk.Frame):
    def __init__(self, parent, controller, show_settings):
        ...
        self.current_time = tk.StringVar(value=f"{controller.timer.get()}:00")
    ...
    def reset_timer(self):
        self.stop_timer()
        timer_time = self.controller.timer.get()
        current_time = self.current_time.set(f"{int(timer_time):02d}:00")
    ...
    def decrement_time(self):
        ...
        if self.timer_running and current_time != "00:00":            
            ...
            if next_up == "Working":
                timer_time = int(self.controller.timer.get())
                self.current_time.set(f"{timer_time:02d}:00")
